...
function Home() {
  ...
  useEffect(() => {
    fetch('mymoviz-backend-beta-three.vercel.app/movies')
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview} voteAverage={data.vote_average} voteCount={data.vote_count} poster={`https://image.tmdb.org/t/p/w500${data.poster_path}`} />;
  });
